...
const ReviewHeader = ({ toilet, onSortChange, currentSort = 'latest' }) => {
  ...
  const renderStars = (rating) => {
    return Array.from({ length: 5 }, (_, index) => {
      ...
      return <StarComponent key={index} className="w-[29px] h-[29px]" />;
    });
  };
        ...
        <div className="flex items-center gap-8">
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <span className="text-heading3-bold text-black">
                {toilet.rating.avg_rating}
              </span>
            </div>
          </div>
          <span className="text-body1 text-black">
            ({toilet.rating.total_reviews}건의 리뷰)
          </span>
        </div>
      </div>
