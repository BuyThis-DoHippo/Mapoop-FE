/**
 * 브라우저에서 사용자의 현재 위치를 가져오는 유틸리티 함수들
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        console.log('현재 위치 조회 성공:', location);
        resolve(location);
      },
      (error) => {
        console.warn('위치 조회 실패:', error.message);
        
        // 사용자가 위치 권한을 거부했거나 오류 발생 시 기본 위치 사용
        switch (error.code) {
...
/**
 * 위치 권한 요청 및 위치 조회
 * @param {Function} onSuccess - 위치 조회 성공 시 콜백
 * @param {Function} onError - 위치 조회 실패 시 콜백
 */
